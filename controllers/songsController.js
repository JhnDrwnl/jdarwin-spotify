...
exports.getAllSongs = async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM tblmusic');
...
exports.searchSong = async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT * FROM tblmusic WHERE title LIKE ? OR artist LIKE ?', [`%${query}%`, `%${query}%`]);
        res.render('songs', { songs: rows });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
...
exports.playSong = async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await db.query('SELECT * FROM tblmusic WHERE id = ?', [id]);
        if (rows.length > 0) {
            const song = rows[0];
            const albumArtUrl = await getAlbumArt(song.title, song.artist);

             const [previousSong] = await db.query('SELECT id FROM tblmusic WHERE id < ? ORDER BY id DESC LIMIT 1', [id]);
             const [nextSong] = await db.query('SELECT id FROM tblmusic WHERE id > ? ORDER BY id ASC LIMIT 1', [id]);
...
exports.uploadSong = [
    ...
    async (req, res) => {
        ...
        try {
            const [result] = await db.query('INSERT INTO tblmusic (title, artist, album, songUrl) VALUES (?, ?, ?, ?)', [title, artist, album, songUrl]);
            res.redirect('/songs'); 
        } catch (error) {
            console.error(error);
...
exports.deleteSong = async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('DELETE FROM tblmusic WHERE id = ?', [id]);
